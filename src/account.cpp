...
error_t Account::LoadAccountDB(std::string name) {
  std::ostringstream query;
  query << "SELECT `id`, `name`, `type`, `password`, `premdays`, `lastday` FROM `accounts` WHERE `name` = "
...
error_t Account::LoadAccountDB(uint32_t id) {
  std::ostringstream query;
  query << "SELECT `id`, `name`, `type`, `password`, `premdays`, `lastday` FROM `accounts` WHERE `id` = " << id;
  return this->LoadAccountDB(query);
}
